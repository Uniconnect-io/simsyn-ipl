...
async function run() {
  ...
  const hashedCaptainPwd = await hashPassword('sipl2026');
  const insertCaptain = db.prepare('INSERT OR IGNORE INTO captains (id, name, password, role) VALUES (?, ?, ?, ?)');
  captains.forEach(c => insertCaptain.run(c.id, c.name, hashedCaptainPwd, 'CAPTAIN'));

  // Pre-configured Admin
  const admin = { id: 'a1', username: 'admin', password: 'admin123' };
  const hashedAdminPwd = await hashPassword(admin.password);
  db.prepare('INSERT OR IGNORE INTO admins (id, username, password) VALUES (?, ?, ?)').run(admin.id, admin.username, hashedAdminPwd);
